...
- ✅ **B 원클릭 통합 진입점 완료** (`run_stt_official_benchmark.py`, 732 tests): 새 엔진을 한 명령으로 측정→비교→판정→리포트. 단계 (1)엔진별 N회 전사 →(2)번들 변환 →(3)CI 집계(항목⑤) →(4)lane matrix →(5)regression →(6)decision →(7)리포트. **순수 command 빌더 + 주입 가능 invoke** 구조로 엔진 실제 구동 없이 배선·순서·멱등성을 fixture로 검증(엔진 구동은 실측 환경). D-b1(`.py` 오케스트레이터)·D-b2(출력 존재 시 skip=재개)·D-b3(N 잠정 5). **product_path 기본 OFF(비교/랭킹)**: 실측 진단 결과 product-path 하니스가 under-transcribe(413 vs 1813자, CER 0.89 vs 0.49)해 기본으로 두면 비교가 degenerate해진다. 본 목적이 "엔진 우열 신뢰성 비교"이므로 기본은 realistic CER(non-product-path), `--product-path`는 adoption opt-in(전 엔진 product_path → candidate `default_allowed` 도달 + benchmark_kind 통일로 비교가능). alias 적용 시 bundle의 canonical engine_id로 매칭(`canonical_engine`). 실측: speech_analyzer 0.286 vs whisper_accurate 0.466(CI±0.020) → significant_improvement, decision=experimental_flag_only.
