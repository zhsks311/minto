...
- `claude -p`(--print) 헤드리스 모드가 TTY 없이 동작하고, `--output-format json`(`.result` 필드), `--model`, `--system-prompt`, `--tools ""`(도구 0)로 순수 텍스트 in→out 호출이 가능하다.
...
- **적용 범위는 요약·답변 생성에 한정**한다(저빈도·지연 허용). **실시간 교정에는 쓰지 않는다** — 호출마다 Node 프로세스 기동(~수백ms)과 구독 rate limit이 스트리밍 교정 배치에 부적합.
...
- **호출 형태**: 전사·프롬프트는 **stdin으로만** 전달한다(argv 금지 — `ps`로 원문 노출; **임시파일 경로 전달도 금지** — 원문 디스크 잔존). 작업별 지시는 `--system-prompt`로 주입. `--output-format json` 파싱. `--model`로 모델 선택(설정값). 도구 차단 플래그(`--tools ""` vs `--disallowedTools "*"`)는 실 CLI 동작으로 확정.
...
- **fail-soft**: 미설치/미인증/timeout/비정상 종료는 `LLMProviderError.notConfigured`/`.unauthorized`/`.network`로 매핑한다. 요약·답변 실패가 전사·저장을 망치지 않는다.
...
- `claude` 설치·로그인된 사용자는 **키 입력 0으로** 요약·답변 AI를 쓰고, 기존 Claude 구독에 과금된다.
- Provider 추상화 재사용 → 교정/요약/답변 독립 설정 체계와 자연스럽게 통합(요약·답변에만 선택 가능).
...
- **지연·rate limit**: Node 기동 오버헤드 + 구독 동시호출 제한 → 실시간 교정 부적합(그래서 범위 제한).
...
- 교정/요약/답변 provider는 독립 키(`llmProvider`/`llmSummaryProvider`/`meetingSearchAnswerProvider`)라, 새 provider를 **요약·답변에서만** 선택 가능하게 노출한다.
...
- **수동 QA**: `claude` 미설치 / 설치+미로그인 / 설치+로그인 세 상태에서 요약·답변 동작·에러 메시지 확인. 설정 "연결 확인" 동작·대기 카피.
- **보안/로그 점검**: 전사 원문이 argv·임시파일·로그 어디에도 안 남는지, 자격증명 미로그. 성공 로그=provider/model/exit code, 실패=stderr prefix만(CLAUDE.md 로깅 규칙). Process env에서 `ANTHROPIC_API_KEY` 제거 확인.
- **경계 점검**: UI가 provider request를 조립하지 않고 use-case가 소유하는지. 교정 picker에 미노출(Registry capability).
