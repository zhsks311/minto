...
- Local LLM:
  ...
  - 설치된 후보 중 `llama3.1:8b`는 가장 빠른 full-case run이지만 correction recall `0.75`, grounded answer recall `0.333`이라 기본값 후보는 보류한다. 다음 기본값 판단은 추가 모델 설치 또는 prompt 조정 benchmark 후 진행한다.
