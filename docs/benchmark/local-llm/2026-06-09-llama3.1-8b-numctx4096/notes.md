# llama3.1:8b num_ctx=4096 benchmark note

작성일: 2026-06-09
결론: latency와 JSON 형식은 좋지만 Minto 기본 로컬 LLM 후보로는 보류한다.
...
python3 scripts/run_local_llm_benchmarks.py \
  ...
  --repeat 1 \
  --server-pid 58693 \
  --output-root docs/benchmark/local-llm/2026-06-09-llama3.1-8b-numctx4096 \
  --fail-fast
```
...
- Mean latency는 `6.894s`, max latency는 `10.733s`다.
- Summary JSON case는 valid JSON과 required field recall `1.0`을 기록했다.
- Grounded answer case는 term recall `0.3333333333333333`을 기록했다.
...
- `llama3.1:8b`는 timeout 없이 빠르게 응답했고 summary JSON 구조도 지켰다.
- 다만 교정 case에서 `PDCR-2901`, `Liquibase`, `Confluence`, `dry-run` 보존이 모두 실패했다.
- Grounded answer도 `15:00`만 보존해 source-time 보존 외 핵심 문구 보존이 약했다.
