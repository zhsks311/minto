# deepseek-r1:8b num_ctx=4096 benchmark note

작성일: 2026-06-09
결론: timeout은 해소됐지만 Minto 기본 로컬 LLM 후보로는 보류한다.
...
- Mean latency는 `29.916s`, max latency는 `50.723s`다.
- Summary JSON case는 valid JSON과 required field recall `1.0`을 기록했다.
- Grounded answer case는 term recall `1.0`을 기록했다.
- Correction case는 응답은 받았지만 expected domain term recall이 `0.0`이다.
- `ollama ps` 확인 시 context는 `4096`, model size는 `5.9 GB`, processor는 `100% GPU`였다.
- `server_rss_peak_mb`는 `ollama serve` PID 기준이라 model runner 전체 메모리로 해석하지 않는다.

## 해석

- `num_ctx=4096`은 이전 `context=131072` timeout 문제를 해소했다.
...
- 현재 결과만으로 `deepseek-r1:8b`를 Minto 기본 로컬 LLM 후보로 올리지 않는다.
