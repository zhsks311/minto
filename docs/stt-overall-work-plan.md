...
17. Nemotron MLX sidecar는 별도 worker로 benchmark만 붙이고, 앱 기본 엔진 후보와 분리한다. 현재 Swift HTTP client 계약과 dependency-free mock worker는 추가됐고, 다음은 실제 Python/MLX worker를 같은 HTTP 계약에 맞춰 cold/warm/timeout metric을 기록한다.
