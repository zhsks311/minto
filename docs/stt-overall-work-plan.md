...
17. Nemotron MLX sidecar는 별도 worker로 benchmark만 붙이고, 앱 기본 엔진 후보와 분리한다. 현재 Swift HTTP client 계약은 추가됐고, 다음은 Python/MLX worker mock 또는 실제 worker를 붙여 cold/warm/timeout metric을 기록한다.
