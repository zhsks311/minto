...
1. 전체 7개 Silero full-duration 결과의 `segments.md`에서 reference density가 높은 empty final chunk를 probe set으로 고정한다.
2. `WhisperEmptyClipDiagnosticsTests`를 probe set에 맞춰 돌려 logProb threshold, temperature fallback, skip/empty guard 중 어느 축이 빈 출력을 만드는지 확인한다.
