...
- `scripts/summarize_stt_benchmarks.py --write-segments`로 empty final과 high-CER window를 샘플/시간대/duration/reference density/참조문/가설문 기준으로 정렬해 볼 수 있다.
...
5. `segments.md`에서 empty final 상위 구간을 뽑아 같은 시간대의 VAD boundary, duration, reference density를 확인한다.
