...
- Nemotron은 `NemotronSidecarClient`와 `NemotronSidecarTranscriber`까지 추가했지만, 아직 `SpeechEngineID`, 설정 UI, 기본 엔진 fallback에는 등록하지 않았다.
...
- `SpeechTranscriptionEngine`: one-shot final 전사 경계다. WhisperKit, SFSpeech, SpeechAnalyzer final-only가 현재 제품 경로다. Nemotron sidecar는 같은 final-only 형태지만, 지금은 제품 등록 전 내부 adapter까지만 둔다.
...
- Swift 앱은 localhost HTTP, stdin/stdout, 또는 Unix domain socket 중 하나로 요청한다.
  ...
  - 현재는 `NemotronSidecarTranscriber`로 sidecar 결과를 앱의 `TranscriptionResult`로 변환하는 내부 adapter를 추가했다.
...
- `SpeechEngineID`와 설정 UI 등록은 별도 단계다. sidecar 안정성, 전체 CER, memory 기준을 통과하기 전에는 제품 선택지로 노출하지 않는다.
