...
초급 개발자는 각 장의 "한눈에 보는 그림"과 "핵심 용어"를 먼저 읽으면 된다. 중급 개발자는 비교표와 실패 모드를 보면 된다. 고급 개발자는 지표, 모델 구조, 런타임 경계, 검증 방법을 중심으로 보면 된다.
...
embedding은 텍스트를 벡터로 바꾸는 과정이다. 비슷한 의미의 문장은 가까운 벡터가 되기를 기대한다. 하지만 embedding 모델, 차원, 정규화, 거리 함수가 다르면 결과도 달라진다. 그래서 embedding 검색은 모델명만 고르는 문제가 아니라 index 설계와 평가 문제다.
...
음성 AI에서는 소리 특징을 넣고, 글자·speaker activity·embedding을 출력한다.  
