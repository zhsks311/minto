...
## 6.1 Step 4 실측 결과 (2026-06-29)

하니스 `Tests/MintoTests/SentenceSplitterEvalTests.swift`(`RUN_SPLITTER_EVAL=1`). 저장 회의 `629FD9B4`(34세그먼트·words 1106개·4화자) + 아카이브 오디오로 VBx 1회 고정, matcher-only vs +splitter 비교.

| VBx 카운트 | matcher 세그먼트 | matcher 화자 | +splitter 세그먼트 | +splitter 화자 | 붕괴 세그먼트 |
|---|---|---|---|---|---|
| exactN=4 | 34 | **3** | 118 | **4** | 5 |
| auto(=4) | 34 | 4 | 116 | 4 | 4 |

해석(효과 분리):
- **문장 단위 + 붕괴 교정은 항상 발현**: 두 경우 모두 한 청크에 2명 이상이 섞인 4~5개 세그먼트를 분리하고, 34→약117 세그먼트(평균 9.4단어=적절한 문장 입도, 과분할 아님).
- **화자 수 복원은 타임라인 의존적**: exactN=4에선 매처가 3명으로 붕괴→splitter가 4명 복원. auto에선 매처가 우연히 4명 유지→복원 +0. 즉 "사라진 화자 복원"은 보너스, "문장마다 화자 정확도"가 핵심 가치.
- **word 드리프트 전제 충족**: 범위이탈 11/1106(1%)·비단조 5/1106(0.5%). 이 녹음에선 word 타임스탬프가 정렬에 충분.

### 6.2 e2e 임포트 검증 (2026-06-29)

하니스 `Tests/MintoTests/SentenceSplitterE2EImportTests.swift`(`RUN_SPLITTER_E2E=1`). `MeetingFileImportUseCase` 실제 경로를 짧은 다화자 클립에 끝까지 실행했다. 교정·요약·저장은 스텁(네트워크/부수효과 차단), STT(WhisperKit)·diarization(VBx)·SentenceSpeakerSplitter는 실제 경로.

실행 대상: `/tmp/splitter-eval/clip5.wav`(5인 토론 원본 앞 4분), `EXPECTED_SPEAKERS=5`.

결과:
- import 저장 record transcript: **46세그먼트**
- words 보유: **46/46**
- 총 word: 452, 평균 **9.8 words/segment**
- 저장 transcript 내 화자 전환: 2회, distinct speakers=3(앞 4분에 실제 발화한 화자 기준)
- 처음 12개 덤프에서 `화자 1 → 화자 2` 전환이 저장 transcript에 반영됨

해석: 실제 import 파이프라인이 `WhisperKit → TranscriptSpeakerMatcher → SentenceSpeakerSplitter → 저장 record` 순서로 동작하고, GUI 없이도 저장 산출물이 문장 단위 화자 세그먼트로 나오는 것을 확인했다. 4분 클립이라 5명 전체 검증은 아니지만, GUI 확인 없이도 배선 e2e는 닫혔다.

### 6.3 추가 일반화 측정 (2026-06-29)

보안 훅이 회의 오디오/전사 파일을 `/tmp`로 복사하는 동작을 차단해, 원본 앱 데이터 경로를 읽기만 하는 방식으로 측정했다. 추가 3개 저장 회의(JSON+recordings wav+words)로 동일 AB를 실행했다.

| 회의 | 원본 세그먼트 | words | VBx exactN | matcher 화자 | +splitter 화자 | +splitter 세그먼트 | 붕괴 세그먼트 | 드리프트(out/nonmono) |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| DE362... | 13 | 717 | 5 | 4 | 4 | 101 | 6 | 0 / 14 |
| 8599... | 7 | 406 | 5 | 4 | 4 | 59 | 2 | 0 / 9 |
| 95B... | 26 | 1581 | 2 | 2 | 2 | 185 | 5 | 0 / 33 |

해석:
- 추가 3개 모두 **문장 단위 세그먼트 증가**와 **붕괴 세그먼트 분리**가 재현됐다.
- 화자 수 복원은 추가 샘플에서는 +0이었다. 이는 앞서 6.1의 결론과 일치: 화자 수 복원은 타임라인/청크 배치 의존 보너스이고, 안정적 핵심 효과는 "한 청크 안 다화자 발화 분리"다.
- word out-of-bounds는 0, non-monotonic은 2% 내외. 경계 정렬에 쓸 수 있는 수준으로 보인다.

### 6.4 UI polish (2026-06-29)

문장 단위로 세그먼트가 3~8배 늘면서 같은 화자 라벨이 과도하게 반복되는 문제를 줄였다. `MeetingLibraryView.transcriptBlock`에서 현재/이전 화자를 비교해:
- 화자가 **이어지는 문장**은 화자 라벨/재배정 메뉴를 regular weight + 55% opacity로 표시
- 화자가 **바뀌는 문장**은 기존처럼 semibold + 100% opacity로 표시
- 편집 메뉴 접근성은 유지(숨기지 않음). 저장 회의의 화자 재배정 UX를 깨지 않는다.
