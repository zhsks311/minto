...
- **D-b4 product_path 기본 OFF(비교/랭킹), `--product-path`는 adoption opt-in**: 원래 설계엔 없던 결정이며, 실측으로 한 번 더 뒤집혔다. decision gate는 candidate가 `product_path=true`여야 `default_allowed`가 가능하고 regression은 `benchmark_kind`가 같아야 비교하므로 adoption엔 product_path가 필요하다(→ `--product-path`로 전 엔진 적용). 그러나 **실측 진단에서 product-path 하니스가 under-transcribe**(본회의_20260428: hypothesis 413 vs 1813자, CER 0.89 vs 0.49)함을 확인 → 기본으로 두면 모든 비교가 degenerate(rejected)해진다. 본 목적이 "엔진 우열 신뢰성 비교"이므로 **기본은 non-product-path(realistic CER)**, product_path는 검증된 adoption에만 명시적으로 켠다. product-path 하니스의 under-transcribe는 별도 과제(Swift 측정 경로).
