...
- `AudioInputMode`를 추가해 `마이크`, `시스템`, `마이크+시스템` 입력 모드를 표현했다. 단, 1차 UI 선택지는 `마이크`, `시스템`으로 제한했다.
...
- `마이크+시스템`은 실제 mixer 구현 전 `UnavailableAudioSource`로 막는다. Echo cancellation과 sample-accurate mix는 후속 측정 대상으로 둔다.
...
- 권한 없음/권한 있음 상태를 녹음 전 UI에서 더 명확히 보여주는 사전 readiness check
- 실제 화상회의 앱 출력으로 level meter 동작 수동 QA
- `마이크+시스템` mode의 mixer 구현, echo/mix 품질 측정
