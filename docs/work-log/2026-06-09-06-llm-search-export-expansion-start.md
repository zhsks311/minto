...
추가 측정으로 `qwen2.5:3b`를 같은 `num_ctx=4096` 조건에서 실행했다. 평균 latency는 `6.957s`로 빨랐지만 correction term recall이 `0.0`이라 기본 로컬 LLM 후보 승격은 보류했다.
추가 측정으로 `llama3.1:8b`도 같은 `num_ctx=4096` 조건에서 실행했다. 평균 latency는 `6.894s`였지만 correction term recall이 `0.0`이라 기본 로컬 LLM 후보 승격은 보류했다.
이후 runner에 Minto형 회의 주제/용어집 교정 case(`correction_terms_with_context`)를 추가했다. 이 조건에서도 `qwen2.5:3b`는 term recall `0.0`이었고, `llama3.1:8b`는 `0.75`까지 개선됐지만 `Liquibase`를 놓쳐 기본 후보 승격은 보류했다.
추가로 `llama3.1:8b` full-case context benchmark를 실행했다. 평균 latency는 `6.753s`로 설치 모델 중 가장 실용적이지만 correction term recall `0.75`, grounded answer recall `0.333`이라 기본 후보 승격은 계속 보류한다.
같은 조건을 repeat 3으로 재측정하니 9/9 transport/format gate는 통과했고 평균 latency는 `4.288s`였지만 correction term recall은 `0.75`로 고정, grounded answer recall은 `0.333...0.667`로 흔들렸다. 설치된 모델 안에서는 `llama3.1:8b`가 가장 빠른 후보지만 기본값 승격은 보류한다.
프롬프트 보존 규칙을 보강해 `llama3.1:8b`를 세 차례 재측정했다. v1에서 correction recall은 `1.0`으로 개선됐지만 grounded answer recall은 `0.333`이었고, v2는 `0.667`, v3는 `0.667, 1.0, 1.0`까지 개선됐다. 다만 v3 repeat에서도 `parent page`를 한 번 놓쳤으므로 프롬프트 단독 보강은 3회 제한에서 멈추고, 기본값 승격은 계속 보류한다.
이후 raw gate와 app 후처리 gate를 분리해 `llama3.1:8b`를 다시 측정했다. raw output 기준으로는 correction wrapper와 길이 초과 때문에 `default_candidate_ready=false`지만, 앱 correction postprocessor 적용 기준으로는 correction clean/length/term recall, summary JSON validity, answer min recall이 모두 `1.0`이라 `app_default_candidate_ready=true`를 기록했다. 단 평균 latency가 `28.813s`이므로 기본값 승격은 latency 정책과 반복 안정성 판단 후 결정한다.
...
- `Sources/Minto/Services/AnswerPrompt.swift`
  ...
  - 여부 질문에서 결론만 답하지 않고 여러 근거의 선행 조건, 영문 UI 용어, 시간 표기를 보존하도록 prompt 규칙 보강
...
- `Tests/MintoTests/MeetingSearchAnswerServiceTests.swift`
  ...
  - 검색 답변 prompt가 선행 조건과 원문 표기 보존 규칙을 포함하는지 검증 추가
- `Tests/MintoTests/LLMProviderTests.swift`
...
- `scripts/run_local_llm_benchmarks.py`
  ...
  - `correction_terms_with_context` case로 Minto형 회의 주제/용어집 교정 prompt 측정 추가
  - benchmark의 correction/answer prompt를 앱 prompt 보존 규칙과 맞춰 `Liquibase`, `dry-run preview`, `parent page`, 시간 표기 보존을 재측정
  - correction raw output gate와 app postprocess gate를 분리하고, app 후처리 기준 correction recall/clean/length OK를 별도 기록
...
- `docs/benchmark/local-llm/2026-06-09-qwen2.5-3b-numctx4096/`
  - `qwen2.5:3b` 실제 Ollama benchmark 결과와 default-candidate 보류 판단 기록
- `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-numctx4096/`
  - `llama3.1:8b` 실제 Ollama benchmark 결과와 default-candidate 보류 판단 기록
- `docs/benchmark/local-llm/2026-06-09-qwen2.5-3b-correction-context-numctx4096/`
  - `qwen2.5:3b` Minto형 correction context benchmark 결과와 default-candidate 보류 판단 기록
- `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-correction-context-numctx4096/`
  - `llama3.1:8b` Minto형 correction context benchmark 결과와 default-candidate 보류 판단 기록
- `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-fullcase-context-numctx4096/`
  - `llama3.1:8b` Minto형 correction, summary, answer full-case benchmark 결과와 default-candidate 보류 판단 기록
- `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-fullcase-context-repeat3-numctx4096/`
  - `llama3.1:8b` Minto형 full-case repeat-3 benchmark 결과와 default-candidate 보류 판단 기록
- `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-prompt-preservation-repeat3-numctx4096/`
  - prompt preservation v1 benchmark 결과 기록. correction recall은 `1.0`, grounded answer recall은 `0.333`
- `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-prompt-preservation-v2-repeat3-numctx4096/`
  - prompt preservation v2 benchmark 결과 기록. grounded answer recall은 `0.667`
- `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-prompt-preservation-v3-repeat3-numctx4096/`
  - prompt preservation v3 benchmark 결과 기록. 9/9 응답, 평균 latency `4.974s`, correction/summary recall `1.0`, grounded answer recall `0.667...1.0`
- `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-app-postprocess-repeat3-numctx4096/`
  - app correction postprocessor 기준 benchmark 결과 기록. 9/9 응답, 평균 latency `28.813s`, raw gate false, app gate true
- `docs/reports/2026-06-09-file-import-rendered-qa.md`
  - 실제 앱 `파일 가져오기` 버튼으로 `minto-import-qa.wav`를 선택하고 rendered 상태 카드의 `전사 다듬는 중` → `회의록 정리 중` → `회의록 생성 완료` 흐름을 기록
...
- 릴리즈 후보 브랜치: `release/llm-search-export-2026-06-09-rc1`
- 릴리즈 후보 워크트리: `/Users/d66hjkxwt9/Idea/private/minto2-release-rc1`
...
- `feature/related-info-reconnect-status`
  - 관련 문서 검색에서 Notion 또는 Confluence가 `needsReconnect`로 바뀌면 일반 빈 결과보다 재연결 안내를 우선 표시
  - Confluence 검색 401과 Notion 재연결 상태의 `RelatedInfoService` status message 테스트 추가
- `feature/confluence-export-reconnect-qa`
  - Confluence 내보내기의 space 조회 401과 page 생성 401 모두 `needsReconnect` 상태로 남기는 회귀 테스트 추가
- `feature/settings-token-status-no-load-qa`
  - Settings 상태 표시 경로에서 Notion/Confluence token 원문을 읽지 않는 회귀 테스트 추가
- `feature/confluence-export-sheet-handoff-qa`
  - Confluence 내보내기 sheet에서 `needsReconnect` 상태일 때 Settings handoff 안내와 버튼 표시
- `feature/local-llm-deepseek-context-benchmark-qa`
  - `deepseek-r1:8b` Minto형 correction context benchmark 추가. term recall `1.0`, latency `43.994s`, 기본값 후보는 full-case repeat 전 보류
- `feature/local-llm-deepseek-fullcase-benchmark-qa`
  - `deepseek-r1:8b` context correction/summary/answer full-case benchmark 추가. 3/3 응답, summary JSON valid, grounded answer recall `0.333`, 기본값 후보 보류
- `feature/local-llm-benchmark-csv-lf`
  - benchmark runner의 `summary.csv` 생성 line ending을 LF로 고정해 `git diff --check` 실패를 방지
- `feature/settings-delete-ui-qa-evidence`
  - Settings UI file secret store delete QA를 격리 root `/tmp/minto2-dev-secrets-delete-ui-qa-1780990000`에서 확인
  - 사용자가 `API 키 삭제`를 누른 뒤 저장 파일이 제거되고 UI가 `API 키 필요` 상태로 바뀐 것을 확인
- `feature/local-llm-llama-fullcase-benchmark-qa`
  - `llama3.1:8b` full-case context benchmark 추가. 3/3 응답, mean latency `6.753s`, summary JSON valid, correction recall `0.75`, grounded answer recall `0.333`, 기본값 후보 보류
- `feature/local-llm-llama-fullcase-repeat3-qa`
  - `llama3.1:8b` full-case context repeat-3 benchmark 추가. 9/9 응답, mean latency `4.288s`, summary JSON valid, correction recall `0.75`, grounded answer recall `0.333...0.667`, 기본값 후보 보류
- `feature/file-import-rendered-qa`
  - 실제 앱 파일 가져오기 rendered QA 추가. `sf_speech_on_device` + mock local LLM으로 `전사 다듬는 중`, `회의록 정리 중`, `회의록 생성 완료` 상태 카드 확인
...
- 병합 후 검증:
  ...
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-related-info-reconnect-status-test --filter 'RelatedInfoTests|IntegrationReconnectStateTests'` 통과: 40 tests
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-confluence-export-reconnect-test --filter 'ConfluenceExportReconnectTests|IntegrationReconnectStateTests'` 통과: 8 tests
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-settings-token-status-no-load-test --filter IntegrationReconnectStateTests` 통과: 8 tests
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-confluence-export-sheet-handoff-test --filter ConfluenceExportSheetTests` 통과: 1 test
  - `python3 scripts/run_local_llm_benchmarks.py --compatibility ollama --base-url http://127.0.0.1:11434 --model deepseek-r1:8b --cases correction_terms_with_context --num-ctx 4096 --repeat 1 --server-pid 58693 --output-root docs/benchmark/local-llm/2026-06-09-deepseek-r1-8b-correction-context-numctx4096 --fail-fast` 통과: term recall `1.0`, latency `43.994s`
  - `python3 scripts/run_local_llm_benchmarks.py --compatibility ollama --base-url http://127.0.0.1:11434 --model deepseek-r1:8b --cases correction_terms_with_context,summary_json,grounded_answer --num-ctx 4096 --repeat 1 --server-pid 58693 --output-root docs/benchmark/local-llm/2026-06-09-deepseek-r1-8b-fullcase-context-numctx4096 --fail-fast` 통과: 3/3 runs, mean latency `36.874s`, grounded answer recall `0.333`
  - `python3 scripts/run_local_llm_benchmarks.py --mock --model mock-model --cases correction_terms_with_context,summary_json,grounded_answer --repeat 1 --output-root /tmp/minto2-local-llm-bench-csv-lf-test` 통과, `summary.csv` CR 문자 없음
  - `python3 scripts/run_local_llm_benchmarks.py --compatibility ollama --base-url http://127.0.0.1:11434 --model llama3.1:8b --cases correction_terms_with_context,summary_json,grounded_answer --num-ctx 4096 --repeat 1 --server-pid 58693 --output-root docs/benchmark/local-llm/2026-06-09-llama3.1-8b-fullcase-context-numctx4096 --fail-fast` 통과: 3/3 runs, mean latency `6.753s`, grounded answer recall `0.333`
  - `python3 scripts/run_local_llm_benchmarks.py --compatibility ollama --base-url http://127.0.0.1:11434 --model llama3.1:8b --cases correction_terms_with_context,summary_json,grounded_answer --num-ctx 4096 --repeat 3 --server-pid 58693 --output-root docs/benchmark/local-llm/2026-06-09-llama3.1-8b-fullcase-context-repeat3-numctx4096 --fail-fast` 통과: 9/9 runs, mean latency `4.288s`, grounded answer recall `0.333...0.667`
  - Settings UI file secret store delete QA 통과: `/tmp/minto2-dev-secrets-delete-ui-qa-1780990000/com.minto.app.llm-api__llm-api-key-gpt.json` 제거, directory empty, UI `API 키 필요`
  - `swift build --disable-sandbox --scratch-path /tmp/minto2-file-import-rendered-qa-build` 통과
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-file-import-rendered-qa-build --filter MeetingFileImportUseCaseTests` 통과: 10 tests
  - rendered file import QA 통과: `/tmp/minto2-file-import-rendered-qa/import-sfspeech-hits.txt`에서 `전사 다듬는 중`, `회의록 정리 중`, `회의록 생성 완료` 확인
- 릴리즈 후보 검증:
  - `git diff --check` 통과
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-release-rc1-smoke-test --filter 'AudioInputMode|LLMProviderTests|MeetingSearchAnswerService|RelatedInfoTests|IntegrationReconnectStateTests|SecretStore'` 통과: 102 tests
...
- Settings 저장 local model 값이 교정, 요약, 검색 답변 provider 선택과 readiness로 이어지는 smoke test를 추가했다.
- Local LLM provider가 `correction`, `finalSummary`, `answer` use case별 Ollama payload와 token budget을 만드는 smoke test를 추가했다.
...
- `swift test --disable-sandbox --scratch-path /tmp/minto2-local-llm-settings-routing-test --filter LLMProviderTests` 통과: 29 tests
- `swift test --disable-sandbox --scratch-path /tmp/minto2-local-llm-service-smoke-test --filter LLMProviderTests` 통과: 30 tests
- `swift test --disable-sandbox --scratch-path /tmp/minto2-local-llm-answer-e2e-test --filter MeetingSearchAnswerServiceTests` 통과: 13 tests
  - `MeetingSearchAnswerUseCase`가 `LocalLLMProvider`의 Ollama `/api/generate` answer payload로 연결되는지 확인
  - 검색 답변 citation 유지, `num_predict=1800`, configured `num_ctx=4096` 확인
- Settings UI 수동 QA:
  - `로컬 LLM` 선택 시 `Endpoint URL`, `AI 모델`, `Ollama generate`, `API 키는 필요하지 않습니다` 문구가 노출됨
  - 모델 ID가 비어 있으면 `모델 ID 필요` 표시
  - `qwen2.5:3b` 입력 후 `로컬 런타임 설정됨` 표시
- 앱 화면 검색 답변 버튼 UI 시도:
  - `127.0.0.1:18080` mock Ollama endpoint를 띄우고 `meetingSearchAnswerProvider=local`로 임시 설정
  - direct SwiftPM foreground 실행은 유지됨. detached/nohup 직접 실행은 rpath/AMFI 로그가 섞여 UI QA 실행 방식에서 제외
  - `HOME`만으로는 Foundation user-domain Application Support가 격리되지 않아 `CFFIXED_USER_HOME=/tmp/minto2-local-llm-ui-home-1780982611`을 함께 사용
  - 격리 meeting store에 seeded 회의 1건을 넣고 앱 UI에서 `저장된 회의 1개` 확인
  - 검색어 `liquibase` 입력 후 "AI 답변" 카드 버튼을 접근성 `AXPress`로 클릭
  - 화면에 `UI-E2E-LOCAL-LLM-ANSWER: 로컬 검색 답변 성공 [1]` 표시 확인
  - `/tmp/minto2-local-llm-ui-e2e-requests-1780982633.jsonl` 마지막 요청 확인:
    - path `/api/generate`
    - model `minto-ui-e2e`
    - `stream=false`
    - `options.num_predict=1800`
    - `options.num_ctx=4096`
    - prompt에 `질문: liquibase`, `회의 근거:`, `db 스키마 형상 관리`, `change-log-master.xml` 포함
  - `/tmp/minto2-local-llm-ui-answer-window.png`에 rendered answer card와 citation list 캡처
  - 앱/mock server 프로세스 종료 확인
  - 실제 `com.minto.app` UserDefaults는 원복:
    - `meetingSearchAnswerEnabled`, `meetingSearchAnswerProvider`, `localLLMBaseURL`, `localLLMCompatibility`, `localLLMTimeoutSeconds`, `localLLMContextWindow` 없음
    - `localLLMModelID=qwen2.5:3b`
- 파일 import 교정/요약 pipeline QA:
  - 현재 UI에는 교정/요약 단독 버튼이 없고, 파일 가져오기 또는 녹음 종료 pipeline stage로 노출됨
  - `MeetingFileImportUseCaseTests`에 `파일 import UI 상태는 교정과 요약 단계를 순서대로 노출한다` 추가
  - correction service 호출 시점에 use case state가 `.correcting`인지 확인
  - summary service 호출 시점에 use case state가 `.summarizing`인지 확인
  - 교정본 `corrected first`, `corrected second`가 최종 요약 입력 transcript로 전달되는지 확인
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-local-llm-correction-summary-pipeline-test --filter MeetingFileImportUseCaseTests` 통과: 10 tests
  - rendered app QA에서 `파일 가져오기` 버튼으로 `/tmp/minto2-file-import-rendered-qa/minto-import-qa.wav`를 선택
  - 접근성 polling이 상태 카드의 `전사 다듬는 중 1/1`, `회의록 정리 중`, `회의록 생성 완료`를 순서대로 확인
  - 저장된 회의 `파일 import rendered QA`와 `저장된 회의 1개` 렌더 확인
  - `swift build --disable-sandbox --scratch-path /tmp/minto2-file-import-rendered-qa-build` 통과
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-file-import-rendered-qa-build --filter MeetingFileImportUseCaseTests` 통과: 10 tests
- 추가 benchmark 후보 확인:
  - 설치된 Ollama 모델은 `llama3.1:8b`, `qwen2.5:3b`, `deepseek-r1:8b`뿐이라 새 모델 후보는 없음
  - 기존 최고 후보였던 `llama3.1:8b`의 `correction_terms_with_context`를 repeat 3으로 재측정
  - 결과 위치: `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-correction-context-repeat3-numctx4096`
  - 3/3 transport/format gate 통과, mean latency `5.617s`, term recall `0.75`
  - 3회 모두 `PDCR-2901`, `Confluence`, `dry-run`은 유지했지만 `Liquibase`를 누락
  - 결론: 현재 설치 모델 중 최선 후보는 `llama3.1:8b`지만 기본값으로 고정하기에는 terminology recall이 부족함

### 남은 항목

- 추가 Local LLM 후보 모델 설치 후 한국어 회의 교정 품질, 요약 구조화 성공률, 검색 답변 근거 충실도, latency, RAM benchmark
...
- `swift build --disable-sandbox --scratch-path /tmp/minto2-mixed-audio-build` 통과
- 실제 앱 setup UI QA:
  - `CFFIXED_USER_HOME=/tmp/minto2-system-audio-ui-home-1780984552 HOME=/tmp/minto2-system-audio-ui-home-1780984552 .build/debug/minto2`로 UserDefaults/Application Support를 격리해 실행
  - `새 회의`에서 회의 시작 sheet를 열고 `시스템` 입력 선택
  - `화상회의 상대방 소리`, `시스템 입력 가능`, `화상회의 상대방 소리를 입력으로 받을 준비가 됐습니다.` 표시와 `녹음 시작` 활성화 확인
  - `마이크+시스템` 입력 선택
  - `내 목소리와 상대방 소리`, `마이크+시스템 입력 가능`, `마이크와 시스템 사운드를 함께 입력합니다. Echo cancellation은 적용하지 않습니다.` 표시와 `녹음 시작` 활성화 확인
  - `/tmp/minto2-system-audio-setup-mixed-ready.png`에 rendered setup window 캡처
  - QA 후 direct SwiftPM app process 종료 확인
  - 현재 macOS 환경은 이미 화면/시스템 오디오 권한이 허용된 상태라 권한 없음 warning과 설정 열기 동작은 이번 run에서 재현하지 않음
- System audio live capture opt-in QA:
  - `SystemAudioLiveTests` 추가
  - 기본 실행에서는 `RUN_SYSTEM_AUDIO_LIVE_TEST=1`이 없으면 즉시 skip
  - opt-in 실행에서는 테스트가 생성한 WAV를 `/usr/bin/afplay` 별도 프로세스로 재생
  - `SystemAudioSource`가 buffer callback에서 1,600 samples 이상을 받고 level callback이 0.001 초과로 갱신되는지 확인
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-system-audio-live-default-test --filter 'SystemAudioLiveTests|AudioInputMode'` 통과: 14 tests
  - `RUN_SYSTEM_AUDIO_LIVE_TEST=1 swift test --disable-sandbox --scratch-path /tmp/minto2-system-audio-live-default-test --filter SystemAudioLiveTests` 통과: 1 test
- Mixed input ViewModel/VAD pipeline QA:
  - `ViewModel은 혼합 입력 source buffer를 VAD pipeline으로 전달한다` 추가
  - `startNewRecordingSession(inputMode: .mixed)`가 `.mixed` source를 요청하고 기존 source를 정지하는지 확인
  - 선택된 source buffer `[0.2, -0.2, 0.4]`가 VAD `process(samples:)`로 전달되는지 확인
  - `swift test --disable-sandbox --scratch-path /tmp/minto2-mixed-audio-viewmodel-pipeline-test --filter AudioInputMode` 통과: 14 tests

### 남은 항목

- 권한 없음 상태에서 readiness warning, `녹음 시작` disabled, 시스템 설정 열기 동작 확인
- 권한 거부 후 허용 상태로 돌아왔을 때 readiness와 level meter 갱신 확인
- 실제 화상회의 앱에서 source-specific system output capture 확인. 별도 프로세스 system output callback은 opt-in live test로 검증됨
- 실제 마이크와 화상회의 앱 출력을 동시에 넣은 `마이크+시스템` VAD/STT 결과 수동 QA. ViewModel 경계의 mixed source buffer to VAD handoff는 자동 검증됨
...
- 기본 LLM API key, OAuth token, Confluence token backend가 실제 프로세스 env의 `MINTO_DEV_SECRET_STORE=file`을 따라 file store를 쓰는 smoke test를 추가했다.
...
- `swift test --disable-sandbox --scratch-path /tmp/minto2-secret-store-process-env-test-2 --filter SecretStore` 통과: 7 tests
- `MINTO_DEV_SECRET_STORE=file MINTO_DEV_SECRET_STORE_ROOT=/tmp/minto2-dev-secrets-process-env-qa swift test --disable-sandbox --scratch-path /tmp/minto2-secret-store-process-env-env-test --filter SecretStore` 통과: 7 tests
- `/tmp/minto2-dev-secrets-process-env-qa` 확인: 테스트 저장 파일은 delete 후 남지 않음
- `MINTO_DEV_SECRET_STORE=file MINTO_DEV_SECRET_STORE_ROOT=/tmp/minto2-dev-secrets-ui-qa ./scripts/dev.sh run` 확인: build, signing, run이 app initialization까지 도달
- `MINTO_DEV_SECRET_STORE=file MINTO_DEV_SECRET_STORE_ROOT=/tmp/minto2-dev-secrets-ui-qa-rc1 ./scripts/dev.sh run` 확인: build, signing, run이 app initialization까지 도달
- Settings UI `GPT API`에서 테스트 값 `minto-ui-qa-not-a-secret` 저장 후 `API 키 저장됨` 표시
- `/tmp/minto2-dev-secrets-ui-qa-rc1/com.minto.app.llm-api__llm-api-key-gpt.json` 생성 확인, 파일 권한 `-rw-------`, 저장 파일명에 secret 원문 미포함
- 같은 `MINTO_DEV_SECRET_STORE_ROOT`로 앱 재시작 후 Settings UI가 `API 키 저장됨` 상태를 다시 표시해 load 확인
- `HOME=/tmp/minto2-settings-delete-ui-home-1780990000 CFFIXED_USER_HOME=/tmp/minto2-settings-delete-ui-home-1780990000 MINTO_DEV_SECRET_STORE=file MINTO_DEV_SECRET_STORE_ROOT=/tmp/minto2-dev-secrets-delete-ui-qa-1780990000 ./scripts/dev.sh run` 확인: build 완료 후 signing certificate `Minto2 Dev` 없음으로 run 중단
- 같은 env에서 direct `.build/debug/minto2` 실행 후 Settings UI `GPT API`가 `API 키 저장됨` 상태로 시작하는 것 확인
- 사용자가 `API 키 삭제`를 누른 뒤 `/tmp/minto2-dev-secrets-delete-ui-qa-1780990000/com.minto.app.llm-api__llm-api-key-gpt.json` 제거, directory empty, Settings UI `API 키 필요` 표시 확인
- `git diff --check` 통과

### 남은 항목

- SecretStore 개발 모드 범위의 Settings UI save, load, delete QA는 완료. 실제 macOS Keychain prompt count는 Keychain reconnect UX 남은 항목으로 추적한다.
...
- Settings Confluence `연동 해제` 수동 QA에서 file SecretStore token 파일 삭제는 확인됐지만, URL/email metadata가 prefs에 남는 cleanup gap을 발견했다.
- `ConfluenceService.disconnect()`를 추가해 token 삭제와 `confluenceBaseURL`/`confluenceEmail` 제거를 같은 서비스 경로로 모았다.
- Settings `연동 해제` 버튼은 `confluence.disconnect()`를 호출하고 임시 token 입력값도 비운다.

### 검증

- 사용자가 isolated file-store Settings에서 `연동 해제`를 누른 뒤 `/tmp/minto2-reconnect-delete-rendered-qa/dev-secrets/com.minto.app.oauth__confluence.json` 삭제 확인
- 기존 구현에서는 `/tmp/minto2-reconnect-delete-rendered-qa/home/Library/Preferences/com.minto.app.plist`에 `confluenceBaseURL`/`confluenceEmail`이 남는 것을 확인
- `swift test --disable-sandbox --scratch-path /tmp/minto2-reconnect-delete-rendered-qa-build --filter IntegrationReconnectStateTests` 통과: 9 tests
- `git diff --check` 통과

### 남은 항목

- Direct SwiftPM executable Settings UI는 seeded file-store token과 prefs가 있어도 Confluence를 `미연동`으로 렌더했다. 실제 signed app bundle 기준의 Confluence rendered reconnect/delete QA는 signing certificate 또는 앱 번들 실행 환경에서 다시 확인해야 한다.

## Local LLM benchmark evidence output

상태: 구현/검증 완료

### 변경 요약

- `scripts/run_local_llm_benchmarks.py`의 per-case metric에 capped `output_preview`, `output_sha256`, `output_preview_truncated`를 추가해 누락 term 원인을 재실행 없이 확인할 수 있게 했다.
- `summary.csv`는 `output_chars`, `output_sha256`, `found_terms`, `missing_terms`를 포함한다.
- `summary.md`는 `Missing Terms` 열을 표시한다.
- runner 문서와 local LLM benchmark 판정 기준에 출력 증거화 계약을 추가했다.

### 검증

- `python3 -m py_compile scripts/run_local_llm_benchmarks.py` 통과
- `python3 scripts/run_local_llm_benchmarks.py --mock --model mock-model --cases grounded_answer --num-ctx 4096 --output-root /tmp/minto2-local-llm-benchmark-output-evidence-mock` 통과
- `/tmp/minto2-local-llm-benchmark-output-evidence-mock/grounded_answer-0.json`에서 `output_preview`, `output_sha256`, `found_terms`, `missing_terms` 기록 확인
- `/tmp/minto2-local-llm-benchmark-output-evidence-mock/summary.csv`에서 found/missing term 열 확인
- `git diff --check` 통과

### 남은 항목

- 이번 변경은 기본 모델 승격이 아니라 평가 증거화다. 후속 gate breakdown과 real benchmark로 기본값 판단을 이어간다.

## Local LLM benchmark gate breakdown

상태: 구현/검증 완료

### 변경 요약

- `summary.json`에 `by_case_type`과 `default_candidate_gate`를 추가해 correction, summary, answer 평가를 분리했다.
- `summary.md`에 `Default Candidate Gate`, `Case Type Summary`, `Case Runs` 섹션을 나눠 기본값 승격 차단 원인을 use case별로 확인할 수 있게 했다.
- default candidate gate는 real non-mock run, correction/summary/answer coverage, transport pass, correction-only output cleanliness, per-type minimum recall, summary JSON valid rate를 함께 본다.
- runner 문서와 benchmark 판정 기준에 gate breakdown 계약을 추가했다.

### 검증

- `python3 -m py_compile scripts/run_local_llm_benchmarks.py` 통과
- `python3 scripts/run_local_llm_benchmarks.py --mock --model mock-model --cases all --num-ctx 4096 --output-root /tmp/minto2-local-llm-benchmark-gate-breakdown-mock` 통과
- `/tmp/minto2-local-llm-benchmark-gate-breakdown-mock/summary.json`에서 `by_case_type.answer`, `by_case_type.correction`, `by_case_type.summary`, `default_candidate_gate` 확인
- mock run은 coverage/transport/recall이 모두 충족돼도 `real_run=false`라 `default_candidate_ready=false`인 것을 확인
- `git diff --check` 통과

### 남은 항목

- 실제 기본값 판단은 `llama3.1:8b` gate breakdown repeat-3 benchmark로 이어서 기록한다.

## Local LLM llama3.1:8b gate benchmark

상태: 구현/검증 완료

### 변경 요약

- `llama3.1:8b`를 보강된 gate breakdown runner로 real Ollama repeat-3 재측정했다.
- correction output cleanliness를 default candidate gate에 추가했다. term recall이 `1.0`이어도 설명 문구, `출력:` 접두어, 줄바꿈, `→` 치환 설명이 있으면 gate를 통과하지 않는다.
- benchmark 결과를 `docs/benchmark/local-llm/2026-06-09-llama3.1-8b-gate-breakdown-repeat3-numctx4096/`에 기록했다.

### 검증

- `python3 -m py_compile scripts/run_local_llm_benchmarks.py` 통과
- `python3 scripts/run_local_llm_benchmarks.py --mock --model mock-model --cases all --num-ctx 4096 --output-root /tmp/minto2-local-llm-correction-clean-gate-mock` 통과
- `python3 scripts/run_local_llm_benchmarks.py --compatibility ollama --base-url http://127.0.0.1:11434 --model llama3.1:8b --cases correction_terms_with_context,summary_json,grounded_answer --num-ctx 4096 --repeat 3 --server-pid 58693 --output-root docs/benchmark/local-llm/2026-06-09-llama3.1-8b-gate-breakdown-repeat3-numctx4096 --fail-fast` 통과: 9/9 runs, mean latency `15.905s`
- `summary.json` 확인: correction clean rate `0.0`, answer min recall `0.667`, `default_candidate_ready=false`
- `metrics.jsonl` 확인: correction repeat 3회 모두 `multiline_output`, `marker:출력:`, `marker:교정합니다`, `marker:→`로 clean gate 실패
- `git diff --check` 통과

### 남은 항목

- `llama3.1:8b` 기본값 승격은 보류한다. 다음 후보 판단은 추가 모델 설치, correction 출력 후처리, 또는 correction 전용 prompt 재설계가 필요하다.

## Correction output postprocessor

상태: 구현/검증 완료

### 변경 요약

- `CorrectionOutputPostprocessor`를 추가해 명확한 `출력:`, `교정 결과:`, `교정본:` wrapper가 있는 correction 응답에서 실제 교정문만 추출한다.
- 마커가 없는 multi-line 응답은 임의로 줄이지 않고 그대로 보존한다.
- `LLMCorrectionService.correct`가 provider 응답을 반환하기 전에 postprocessor를 적용한다.

### 검증

- `swift test --disable-sandbox --scratch-path /tmp/minto2-correction-output-postprocessor-test --filter CorrectionPromptTests` 통과: 10 tests
- `git diff --check` 통과

### 남은 항목

- 이 변경은 앱 correction 응답 후처리다. benchmark runner의 raw model gate는 모델 원응답 품질 추적용으로 남긴다.

## Local LLM app postprocess benchmark

상태: 구현/검증 완료

### 변경 요약

- benchmark runner가 correction output에 대해 raw gate와 app postprocess gate를 함께 기록한다.
- raw gate는 모델 원응답의 wrapper/설명/길이 초과를 그대로 평가한다.
- app gate는 앱 correction postprocessor와 같은 보수적 wrapper stripping 후 correction term recall, clean rate, length OK rate를 평가한다.
- `llama3.1:8b` app-postprocess repeat-3 결과를 benchmark evidence로 기록했다.

### 검증

- `python3 -m py_compile scripts/run_local_llm_benchmarks.py` 통과
- `python3 scripts/run_local_llm_benchmarks.py --mock --model mock-model --cases all --num-ctx 4096 --output-root /tmp/minto2-local-llm-app-postprocess-final-mock-1781004000` 통과
- `/tmp/minto2-local-llm-app-postprocess-final-mock-1781004000/summary.json` 확인: mock app correction clean/length OK rate `1.0`, mock run이라 `app_default_candidate_ready=false`
- `python3 scripts/run_local_llm_benchmarks.py --compatibility ollama --base-url http://127.0.0.1:11434 --model llama3.1:8b --cases correction_terms_with_context,summary_json,grounded_answer --num-ctx 4096 --repeat 3 --server-pid 58693 --output-root docs/benchmark/local-llm/2026-06-09-llama3.1-8b-app-postprocess-repeat3-numctx4096 --fail-fast` 통과: 9/9 runs, mean latency `28.813s`
- `summary.json` 확인: raw `default_candidate_ready=false`, app `app_default_candidate_ready=true`
- `git diff --check` 통과

### 남은 항목

- 앱 후처리 기준 품질 gate는 통과했지만 평균 latency가 `28.813s`다. 기본값 승격은 latency 정책과 반복 안정성 기준을 정한 뒤 결정한다.
