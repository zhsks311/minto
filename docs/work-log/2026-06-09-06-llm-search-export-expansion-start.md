...
- 앱 화면 검색 답변 버튼 UI 시도:
  ...
  - direct SwiftPM `minto2` app process가 UI 클릭 전 두 차례 종료되어 rendered button click 증거는 확보하지 못함
  - 임시 UserDefaults는 `/tmp/minto2-com.minto.app-before-local-llm-ui-e2e.plist` 기준으로 복원

### 남은 항목

- Settings UI에서 local provider 선택 후 앱 화면의 교정, 요약, 검색 답변 버튼 클릭 수동 QA. Search answer use case to Local LLM payload는 자동 검증됐지만 rendered app button click은 direct SwiftPM app process 종료로 별도 확인 필요
