...
- Settings UI에서 local provider 선택과 상태 문구 수동 QA
...
- Settings UI 저장/load/delete는 미확인: `computer-use`가 SwiftPM direct `minto2` process를 target하지 못했고, shell `System Events` window inspection은 accessibility error `-1728`로 차단됨
- `git diff --check` 통과

### 남은 항목

- 실제 개발 실행에서 `MINTO_DEV_SECRET_STORE=file MINTO_DEV_SECRET_STORE_ROOT=/tmp/minto-dev-secrets`로 Settings UI 저장, 앱 재시작 후 load, delete 수동 QA. Env app launch는 확인됐고, UI save/load/delete만 남음
