...
- 이번 변경은 기본 모델 승격이 아니라 평가 증거화다. 다음 기본값 판단은 추가 모델 설치 또는 답변 생성 후처리/평가 기준 분리로 진행한다.
...
- default candidate gate는 real non-mock run, correction/summary/answer coverage, transport pass, per-type minimum recall, summary JSON valid rate를 함께 본다.
...
- 실제 기본값 판단은 real Ollama run의 gate breakdown으로 다시 기록해야 한다. 현재 설치 후보의 `llama3.1:8b` 기본값 승격 보류 판단 자체는 유지한다.
