...
- 사용자 동작 단위 기능을 추가/수정하면 시작(`.info`)·성공(`.info`)·실패(`.error`) 로그를 함께 추가한다. `.debug`는 배포에서 영속이 보장되지 않으므로 핵심 이벤트에 쓰지 않는다.
- 금지 값: 전사·요약·주제·검색어·용어집 내용·문서 문맥·프롬프트·토큰/키·API 응답 body 원문·홈 디렉터리 포함 절대경로. 허용 값: 개수/길이, enum 케이스명, provider/모델/엔진 식별자, HTTP status, `lastPathComponent`, 에러 설명.
