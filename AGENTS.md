...
배포 바이너리에서 사용자 문제를 추적할 수 있도록 os.Logger 기반 이벤트 로그를 유지한다.

- 로그는 `Log.<category>`(`Sources/Minto/Services/Log.swift`)만 사용. `print`/`fputs`/`NSLog` 금지 (stderr는 배포 시 소실).
- 카테고리: app, stt, vad, audio, correction, summary, store, search, importer, oauth, report. 새 서브시스템이 생기면 Log.swift에 카테고리를 추가한다.
- 사용자 동작 단위 기능을 추가/수정하면 시작(`.info`)·성공(`.info`)·실패(`.error`) 로그를 함께 추가한다. `.debug`는 배포에서 영속이 보장되지 않으므로 핵심 이벤트에 쓰지 않는다. fail-soft 기능(실패가 화면에 안 보이는 기능)의 실패는 반드시 `.error` — 로그가 유일한 관측 수단이다.
- 결정과 설정도 이벤트다: 동작을 바꾸는 설정 변경(모델/provider 선택)과 런타임 결정(모델 폴백, provider 라우팅)을 기록하고, 성공 로그에 실제 적용된 값(`model=` 등)을 포함한다.
- 실패 로그에는 판별 증거를 남긴다: HTTP 실패는 status + 에러 응답 본문 prefix(200자). 에러 본문은 서버 거부 사유라 민감값이 아니며, `bodyLen`만 남기면 사유 구분이 안 된다. status code만으로 에러를 분류하지 말고 본문 사유로 구분한다.
- 금지 값: 전사·요약·주제·검색어·용어집 내용·문서 문맥·프롬프트·토큰/키·정상(2xx) API 응답 body 원문(2xx 파싱 실패는 bodyLen + 누락 필드명만 기록)·홈 디렉터리 포함 절대경로. 허용 값: 개수/길이, enum 케이스명, provider/모델/엔진 식별자, HTTP status, 비-2xx 에러 응답 본문 prefix, `lastPathComponent`, 에러 설명.
- privacy 마스킹은 안전장치가 아니다 — "진단 로그 내보내기"가 같은 프로세스의 `composedMessage`(마스킹 미적용 원문)를 파일로 쓴다. 민감값은 마스킹이 아니라 아예 넣지 않는 것으로 막는다. 비민감 값은 `privacy: .public` 명시(누락 시 `<private>`로 가려짐).
